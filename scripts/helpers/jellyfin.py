...
def authenticate():
    ...
    data = {'Username': creds.JELLYFIN_ADMIN_USERNAME, 'Password': creds.JELLYFIN_ADMIN_PASSWORD, 'Pw': creds.JELLYFIN_ADMIN_PASSWORD}
    try:
...
def post(cmd, params, payload):
    return requests.post('{}{}?api_key={}{}'.format(JELLYFIN_URL, cmd, JELLYFIN_API_KEY, ("&" + params if params is not None else "")), json=payload)
...
def getLibraries():
    url = '/Users/{}/Items?{}'.format(str(admin_id), url)
...
def addToPlaylist(playlistId, itemIds):
    ...
    url = '/Playlists/{}/Items?{}'.format(str(playlistId), str(itemList))
    print(url)
    return postWithToken(hdr=token_header, method=url)
